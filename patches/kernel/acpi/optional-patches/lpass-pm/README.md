...
Каталог применяется **после основной ACPI-серии** и разделяет аппаратно проверенное управление LPI clocks от сохранённых кандидатов qc7. В `series` включён только проверенный LPI runtime-PM patch. Жизненный цикл Q6AFE hardware-vote на ноутбуке пока некорректен: ранний vote и поздний devote получали `Unknown cmd`, хотя позднее RX/TX/VA macro были привязаны и звук работал. Поэтому Q6AFE и VA macro patches сохранены для анализа происхождения, но в рабочую TCL-серию не входят. DT-комплект не меняется.

## Состав

| Файл | Назначение и происхождение |
|---|---|
| [0001](0001-lpass-va-macro-disable-clocks-in-suspend.patch) | Оригинал Nikita Travkin `283c8d3f8579`: баланс macro/dcodec clocks в VA macro runtime suspend/resume. Сохранён без изменений и не входит в `series`: полный Q6AFE vote/devote lifecycle ещё не проходит аппаратную проверку. |
| [0002](0002-q6afe-track-dsp-vote-handle.patch) | Логически отдельная адаптация Nikita Travkin `2637dc2cbba0`: сохранить DSP client handle и передать его при unvote. Не входит в `series`, пока реальный vote/devote lifecycle не работает без ошибок firmware. |
...
| [0004](0004-q6afe-compact-vote-response.patch) | Логически отдельная поддержка компактного 32-битного vote-response. Не входит в `series`: подтверждённый ранний ответ TCL firmware был обычным `APR_BASIC_RSP_RESULT`, status `0x16` (`Unknown cmd`) для opcode `0x100f4`, а не компактным успешным ответом. |
...
Отдельная аппаратная проверка Q6AFE уточнила границы проблемы. При раннем probe VA macro запросил блок 3 командой `AFE_CMD_REMOTE_LPASS_CORE_HW_VOTE_REQUEST` (`0x100f4`), ADSP ответил обычным `APR_BASIC_RSP_RESULT` со status `0x16`, драйвер расшифровал его как `Unknown cmd`, а VA probe завершился `-ETIMEDOUT`. После полной готовности APR RX/TX/VA macro оказались привязаны, оба vote clocks имели по три prepare references, и ALSA playback работал. Однако контролируемое снятие RX/TX macro позднее дало два `Unknown cmd 0x100f6` для devote. Поэтому доказано, что lifecycle всё ещё неверен, но недостаточно данных утверждать, что firmware всегда отвергает vote. Hot reload также подтвердил отдельный дефект cleanup: повторный probe RX/TX сообщил `Unbalanced pm_runtime_enable`, а VA module остался in-use. Карта и оба SoundWire slave после штатного перезапуска сервиса были восстановлены; контрольный PipeWire playback на 50% завершился успешно.
