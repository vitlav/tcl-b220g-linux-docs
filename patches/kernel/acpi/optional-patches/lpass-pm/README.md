...
Набор применяется **после основной ACPI-серии**. Он сохраняет исправления qc7 и адаптирует управление clocks к текущему ядру. Применение, сборка и перечисленные ниже проверки кода выполнены; аппаратная проверка новых модулей ещё не выполнена. В основной загрузочный комплект изменения не включены. DT-комплект не меняется.
