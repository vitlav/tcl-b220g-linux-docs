import os, struct, json, base64, hashlib, re
...
for line in Path('/proc/iomem').read_text().splitlines():
    m=re.fullmatch(r'\s*([0-9a-f]+)-([0-9a-f]+) : reserved',line)
    if m: reserved.append((int(m[1],16),int(m[2],16)+1))
if not any(a<=LOW and HIGH<=b for a,b in reserved):
    raise ValueError('Expected ACPI range is not reserved in current iomem')
