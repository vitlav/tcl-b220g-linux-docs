...
if((Get-FileHash $boot).Hash -ne 'C900B48D4B871CE7ACFCB4E00C809415D304046F1EF20AE17D32CA590706D1DE') { throw 'Unexpected previous EFI' }
...
$payload=Join-Path $root 'tcl-probe-isolation'
...
$backup=Join-Path $root 'EFI\BOOT\BOOTAA64-initcall.bak'
...
[ordered]@{Disk=$disks[0].Number;Root=$root;EFIHash=(Get-FileHash $boot).Hash;FilesVerified=$manifest.Count;Backup='EFI/BOOT/BOOTAA64-initcall.bak'} | ConvertTo-Json -Compress
