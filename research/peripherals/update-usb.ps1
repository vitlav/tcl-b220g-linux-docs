...
if((Get-FileHash $boot).Hash -ne '0A575CA6269C6DD4AFA278155F892E32DD57F2AFFB69CA7492044A2EE31A140F') { throw 'Unexpected previous EFI' }
...
$payload=Join-Path $root 'tcl-peripherals'
New-Item -ItemType Directory -Force $payload | Out-Null
foreach($f in $manifest | Where-Object Name -ne 'TCL-PERIPHERALS-BOOTAA64.EFI') {
...
$backup=Join-Path $root 'EFI\BOOT\BOOTAA64-smp.bak'
if(Test-Path $backup) { throw 'Backup already exists; review required' }
Copy-Item $boot $backup
Copy-Item (Join-Path $PSScriptRoot 'TCL-PERIPHERALS-BOOTAA64.EFI') $boot -Force
$expected=($manifest | Where-Object Name -eq 'TCL-PERIPHERALS-BOOTAA64.EFI').SHA256
if((Get-FileHash $boot).Hash -ne $expected) { throw 'EFI write mismatch' }
[ordered]@{Disk=$disks[0].Number;Root=$root;EFIHash=(Get-FileHash $boot).Hash;FilesVerified=$manifest.Count;Backup='EFI/BOOT/BOOTAA64-smp.bak'} | ConvertTo-Json -Compress
