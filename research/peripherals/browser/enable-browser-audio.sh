...
runuser -u tcl-browser -- env XDG_RUNTIME_DIR=/run/tcl-browser pulseaudio  --daemonize=no --exit-idle-time=-1 --use-pid-file=no --realtime=no  --log-target=stderr -nF /var/tmp/tcl-audio2/browser-pulse.pa  > /var/tmp/tcl-audio2/browser-pulse.log 2>&1 &
playpid=$!
ready=no
