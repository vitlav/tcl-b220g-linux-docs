...
insmod ./tcl_wcd_watchdog.ko hold_seconds=0
...
runuser -u tcl-browser -- env XDG_RUNTIME_DIR=/run/tcl-browser pulseaudio  --daemonize=no --exit-idle-time=-1 --use-pid-file=no --realtime=no  --log-target=stderr -nF /usr/local/lib/tcl-audio2/browser-pulse.pa  > /run/tcl-browser/browser-pulse.log 2>&1 &
playpid=$!
ready=no
...
test "$ready" = yes
runuser -u tcl-browser -- env XDG_RUNTIME_DIR=/run/tcl-browser pactl set-sink-volume tcl_speakers 50%
insmod ./tcl_speaker_pa.ko hold_seconds=0
echo FIREFOX_AUDIO_READY
runuser -u tcl-browser -- env XDG_RUNTIME_DIR=/run/tcl-browser pactl list short sinks
