...
test -x /run/tcl-pulse-stage/usr/bin/pulseaudio
test -x /run/tcl-pulse-stage/usr/bin/pactl
...
# Native socket belongs to the same unprivileged user as Firefox.
runuser -u tcl-browser -- env LD_LIBRARY_PATH=/run/tcl-pulse-stage/usr/lib/aarch64-linux-gnu:/run/tcl-pulse-stage/usr/lib/aarch64-linux-gnu/pulseaudio:/run/tcl-pulse-stage/usr/lib/pulse-17.0+dfsg1/modules XDG_RUNTIME_DIR=/run/tcl-browser /run/tcl-pulse-stage/usr/bin/pulseaudio --dl-search-path=/run/tcl-pulse-stage/usr/lib/pulse-17.0+dfsg1/modules  --daemonize=no --exit-idle-time=-1 --use-pid-file=no --realtime=no  --log-target=stderr -nF /var/tmp/tcl-audio2/browser-pulse.pa  > /var/tmp/tcl-audio2/browser-pulse.log 2>&1 &
playpid=$!
ready=no
for n in $(seq 1 60); do
 sleep 1
 if test -S /run/tcl-browser/pulse/native && grep -q 'state: RUNNING' /proc/asound/card0/pcm0p/sub0/status; then ready=yes; break; fi
 kill -0 "$playpid"
done
test "$ready" = yes
runuser -u tcl-browser -- env LD_LIBRARY_PATH=/run/tcl-pulse-stage/usr/lib/aarch64-linux-gnu:/run/tcl-pulse-stage/usr/lib/aarch64-linux-gnu/pulseaudio:/run/tcl-pulse-stage/usr/lib/pulse-17.0+dfsg1/modules XDG_RUNTIME_DIR=/run/tcl-browser /run/tcl-pulse-stage/usr/bin/pactl set-sink-volume tcl_speakers 50%
insmod ./tcl_speaker_pa.ko hold_seconds=900
echo FIREFOX_AUDIO_READY
runuser -u tcl-browser -- env LD_LIBRARY_PATH=/run/tcl-pulse-stage/usr/lib/aarch64-linux-gnu:/run/tcl-pulse-stage/usr/lib/aarch64-linux-gnu/pulseaudio:/run/tcl-pulse-stage/usr/lib/pulse-17.0+dfsg1/modules XDG_RUNTIME_DIR=/run/tcl-browser /run/tcl-pulse-stage/usr/bin/pactl list short sinks
