...
if((Get-FileHash $boot).Hash -ne '8ADF47B1C45E700DD6175B406BCA9C1A02A60C764CD7BC9C7CD1D74C5D9D8C3C') { throw 'Unexpected previous EFI' }
...
$payload=Join-Path $root 'tcl-core-isolation'
...
$backup=Join-Path $root 'EFI\BOOT\BOOTAA64-probe-isolation.bak'
...
[ordered]@{Disk=$disks[0].Number;Root=$root;EFIHash=(Get-FileHash $boot).Hash;FilesVerified=$manifest.Count;Backup='EFI/BOOT/BOOTAA64-probe-isolation.bak'} | ConvertTo-Json -Compress
