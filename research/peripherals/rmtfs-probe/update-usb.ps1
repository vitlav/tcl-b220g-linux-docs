...
if((Get-FileHash $boot).Hash -ne 'E6CC4CE7F08913FB860B6D7BC0751D5C14E1211FE27A782EC5F005058C6BA67C') { throw 'Unexpected previous EFI' }
...
$payload=Join-Path $root 'tcl-rmtfs-probe'
...
$backup=Join-Path $root 'EFI\BOOT\BOOTAA64-wifi-timeline.bak'
...
[ordered]@{Disk=$disks[0].Number;Root=$root;EFIHash=(Get-FileHash $boot).Hash;FilesVerified=$manifest.Count;Backup='EFI/BOOT/BOOTAA64-wifi-timeline.bak'} | ConvertTo-Json -Compress
