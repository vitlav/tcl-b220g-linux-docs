...
if((Get-FileHash $boot).Hash -ne '1C5029398C50A3A8B674B8FA830170A765C8D283EBEBB8FAC1D8351D47E65451') { throw 'Unexpected previous EFI' }
...
$payload=Join-Path $root 'tcl-logging-v2'
...
$backup=Join-Path $root 'EFI\BOOT\BOOTAA64-tlmm-reserved.bak'
...
[ordered]@{Disk=$disks[0].Number;Root=$root;EFIHash=(Get-FileHash $boot).Hash;FilesVerified=$manifest.Count;Backup='EFI/BOOT/BOOTAA64-tlmm-reserved.bak'} | ConvertTo-Json -Compress
