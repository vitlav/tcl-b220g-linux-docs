...
XDG_RUNTIME_DIR=/run/tcl-weston WAYLAND_DISPLAY=wayland-tcl mpv --no-config --fullscreen --vo=gpu-next --gpu-api=opengl --gpu-context=wayland --hwdec=no --ao=alsa --audio-device=alsa/hw:Test,0 --audio-samplerate=48000 --audio-format=s16 --audio-channels=stereo --volume=15 --volume-max=100 --input-ipc-server=/run/tcl-weston/mpv-sound-test.sock "$movie" > /var/tmp/tcl-audio2/movie-mpv.log 2>&1 &
playpid=$!
ready=no
