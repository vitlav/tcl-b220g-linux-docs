...
timeout 10 aplay -D hw:0,0 -t raw -f S16_LE -r 48000 -c 2 -d 6 /dev/zero > /var/tmp/tcl-audio2/card-path-aplay.txt 2>&1 &
playpid=$!
sleep 1
cat /proc/asound/card0/pcm0p/sub0/status
cat /proc/asound/card0/pcm0p/sub0/hw_params
find /sys/kernel/debug/asoc -type f > /var/tmp/tcl-audio2/card-path-debugfs-files.txt
find /sys/kernel/debug/asoc -type f -path '*/dapm/*' | while IFS= read -r f; do
 echo "FILE=$f"
 cat "$f"
done > /var/tmp/tcl-audio2/card-path-dapm.txt
cat /sys/kernel/debug/gpio > /var/tmp/tcl-audio2/card-path-gpio.txt
wait "$playpid"
cat /var/tmp/tcl-audio2/card-path-aplay.txt
echo CARD_TEST_END
