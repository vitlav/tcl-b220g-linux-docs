...
XDG_RUNTIME_DIR=/run/tcl-weston WAYLAND_DISPLAY=wayland-tcl mpv --no-config --msg-level=all=v --cache=yes --cache-secs=60 --demuxer-readahead-secs=60 --demuxer-max-bytes=256MiB --cache-pause-wait=3 --fullscreen --vo=gpu-next --gpu-api=opengl --gpu-context=wayland --hwdec=no --ao=alsa --audio-device=alsa/hw:Test,0 --audio-samplerate=48000 --audio-format=s16 --audio-channels=stereo --volume=50 --volume-max=100 --input-ipc-server=/run/tcl-weston/mpv-sound-test.sock "$movie" > /var/tmp/tcl-audio2/movie-usb-cache60-mpv.log 2>&1 &
playpid=$!
ready=no
n=0
