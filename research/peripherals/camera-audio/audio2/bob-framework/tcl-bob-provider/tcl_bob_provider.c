			...
			struct tcs_cmd *cmd, bool wait_for_ack)
{
	...
	pr_info("TCL_BOB_REQUEST addr=%x value=%u ret=%d\n", cmd->addr, cmd->data, ret);
	return ret;
}
...
static const struct rpmh_vreg_init_data tcl_bob_data[] = {
 RPMH_VREG("bob", "bob%s1", &pmic5_bob, NULL), {}
};
...
static int rpmh_regulator_probe(struct platform_device *pdev)
{
	...
	if (!of_machine_is_compatible("tcl,book14") || cmd_db_read_addr("bobc1") != 0x40400)
  return -ENODEV;
...
static const struct of_device_id rpmh_regulator_match_table[] = {
 { .compatible="tcl,book14-bob-test", .data=tcl_bob_data }, {}
};
