...
static int __init start(void)
{
 ...
 of_node_put(np);
 np=of_find_node_by_path("/soc@0/txmacro@62620000");
 ...
 lookup=clkdev_create(fsgen,"fsgen","62620000.txmacro");
 if (!lookup) { clk_put(fsgen); return -ENOMEM; }
 pr_info("TCL_TX_OVERLAY LPASS codec=%s, VA fsgen provider acquired\n",lpass_macro_get_codec_version_string(lpass_macro_get_codec_version()));
 ...
 pr_info("TCL_TX_OVERLAY id=%d: TX platform node and fsgen alias, persistent until reboot\n",overlay_id);
 return 0;
}
module_init(start);
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("TCL TX macro test; SoC platform node, preserves APR clock provider");
