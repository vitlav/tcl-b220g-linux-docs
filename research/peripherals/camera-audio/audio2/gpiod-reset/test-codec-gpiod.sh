...
test -f "$original"
test ! -d /sys/module/tcl_bob_provider
test ! -d /sys/module/tcl_codec_reset
...
cleanup() {
 ...
 if test -d /sys/module/tcl_gpiod_reset; then rmmod tcl_gpiod_reset; fi
