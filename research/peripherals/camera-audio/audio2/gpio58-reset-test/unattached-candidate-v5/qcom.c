...
static int qcom_swrm_enumerate(struct sdw_bus *bus)
{
	...
	for (i = 1; i <= SDW_MAX_DEVICES; i++) {
		...
		dev_info(ctrl->dev, "TCL ENUM slot=%d addr=%012llx\n",i,addr);
...
static irqreturn_t qcom_swrm_irq_handler(int irq, void *dev_id)
{
	...
	dev_info(ctrl->dev, "TCL IRQ raw=%08x mask=%08x cached_slave=%08x\n",intr_sts,ctrl->intr_mask,ctrl->slave_status);
...
static int __maybe_unused swrm_runtime_resume(struct device *dev)
{
	...
	if (ctrl->clock_stop_not_supported) {
		...
		wait_for_completion_timeout(&ctrl->enumeration,
					    msecs_to_jiffies(TIMEOUT_MS));
		qcom_swrm_get_device_status(ctrl);
		dev_info(dev, "TCL test: enumerate after reset status=%08x\n",ctrl->slave_status);
		qcom_swrm_enumerate(&ctrl->bus);
		sdw_handle_slave_status(&ctrl->bus, ctrl->status);
	} else {
		reset_control_reset(ctrl->audio_cgcr);
...
static int __maybe_unused swrm_runtime_suspend(struct device *dev)
{
	...
	if (!ctrl->clock_stop_not_supported) {
		 ...
		 */
		qcom_swrm_get_device_status(ctrl);
		if ((ret == 0 || ret == -ENODATA) && ctrl->slave_status == 0) {
			dev_info(dev, "TCL test: no hardware slaves, reset on next resume\n");
			ctrl->clock_stop_not_supported = true;
		}
		if (ret < 0 && ret != -ENODATA) {
