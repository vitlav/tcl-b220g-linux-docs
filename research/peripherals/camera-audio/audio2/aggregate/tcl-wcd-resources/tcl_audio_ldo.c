			...
			struct tcs_cmd *cmd, bool wait_for_ack)
{
	...
	pr_info("TCL_LDO_REQUEST addr=%x value=%u ret=%d\n", cmd->addr, cmd->data, ret);
	return ret;
}
...
static const struct rpmh_vreg_hw_data pmic5_bob = {
	...
	.ops = &rpmh_regulator_vrm_bypass_ops,
	.voltage_ranges = (struct linear_range[]) {
		REGULATOR_LINEAR_RANGE(3300000, 0, 0, 0),
	},
	.n_linear_ranges = 1,
	.n_voltages = 1,
...
static const struct rpmh_vreg_init_data tcl_bob_data[] = {
 RPMH_VREG("ldo10", "ldo%s10", &pmic5_pldo_lv, NULL),
 RPMH_VREG("ldo15", "ldo%s15", &pmic5_pldo_lv, NULL), {}
};
...
static int rpmh_regulator_probe(struct platform_device *pdev)
{
	...
	if (!of_machine_is_compatible("tcl,book14") || cmd_db_read_addr("ldoa10") != 0x41d00 || cmd_db_read_addr("ldoa15") != 0x42200)
  return -ENODEV;
...
static const struct of_device_id rpmh_regulator_match_table[] = {
 { .compatible="tcl,book14-audio-ldo-test", .data=tcl_bob_data }, {}
};
MODULE_DEVICE_TABLE(of, rpmh_regulator_match_table);

static struct platform_driver rpmh_regulator_driver = {
	.driver = {
		.name = "tcl-audio-ldo-test",
