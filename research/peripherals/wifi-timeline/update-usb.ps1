...
if((Get-FileHash $boot).Hash -ne 'A284CE394C9F70B615A18C29EB8474D7ECAC562351324BCF06E6F2E576E34630') { throw 'Unexpected previous EFI' }
...
$payload=Join-Path $root 'tcl-wifi-timeline'
...
$backup=Join-Path $root 'EFI\BOOT\BOOTAA64-wifi-services.bak'
...
[ordered]@{Disk=$disks[0].Number;Root=$root;EFIHash=(Get-FileHash $boot).Hash;FilesVerified=$manifest.Count;Backup='EFI/BOOT/BOOTAA64-wifi-services.bak'} | ConvertTo-Json -Compress
