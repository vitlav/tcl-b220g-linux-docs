...
Сводка обновлена 2026-09-25; даты и условия отдельных проверок приведены в тематических разделах. Таблица ниже сохраняет DT-эталон; отдельные ACPI-результаты явно подписаны. Полное сопоставление DT/ACPI и критерии готовности: [план поддержки](roadmap.md).
...
| [Подсветка ACPI](hardware/backlight-acpi.md) | PM6150L SID5/PWM0xbc00: raw0 — максимум, raw511 — темнота; направление подтверждено визуально | Постоянный backlight, корректное выключение и suspend/resume |
...
| [EC/батарея](hardware/ec.md) | DT: прямое чтение и зарядка; ACPI: автоматические BAT0/ADP1 через стандартный power_supply | Проверить разряд/заряд, AC events, UPower, Lid Switch и suspend/resume |
...
| ACPI | Ubuntu 26.04.1 LTS и ALT Linux, USB/rootfs, Wi-Fi/SSH, ввод, MSM/Wayland/Adreno, звук, BAT0/ADP1 | Воспроизводимость полного комплекта, повторные циклы, UFS/видео/capture и suspend/resume — отдельная квалификация |

Текущая ACPI-система использует ALT Linux и `7.2.4-tcl-acpi-public70+`. Её функциональные результаты не означают, что весь новый исходный комплект уже пересобран и повторно проверен: [серия и манифест](../patches/kernel/acpi/README.md).
