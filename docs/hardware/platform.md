...
SMBIOS 3.1 самой машины содержит одну запись Type 17 для распаянной памяти:

| Параметр | Значение |
|---|---|
| Объём | 8192 MB |
| Тип SMBIOS | LPDDR4 (`0x1e`) |
| Форм-фактор | Row Of Chips, `Top - on board` |
| Скорость | 2133 MT/s |
| Настроенная скорость | 2133 MT/s |
| Организация контроллера | 2×16 бит |

Последняя строка взята из [официального описания Snapdragon 7c](https://www.qualcomm.com/content/dam/qcomm-martech/dm-assets/documents/prod_brief_qcom_sd7c.pdf); остальные поля декодированы из живой SMBIOS-таблицы TCL B220G. SMBIOS сообщает LPDDR4 и не позволяет подтвердить суффикс LPDDR4X. Теоретический предел 32-битного интерфейса при 2133 MT/s составляет около 8,53 GB/s; это расчётная величина, а не измеренная пропускная способность. Linux видит около 7,3 GiB после аппаратных и firmware-резервов.
