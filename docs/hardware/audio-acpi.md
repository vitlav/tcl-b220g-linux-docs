...
The speaker PA uses GPIO46/47, controlled by a DAPM speaker widget. During playback both lines go high; they return low when the PCM closes. For orderly mpv stop, fade volume, switch off `Internal Speaker` while PCM remains `RUNNING`, then close the PCM. The ordering and GPIO transitions were verified. A controlled test with one second of digital silence before the tone, PA enable only after PCM reached `RUNNING`, PA disable before PCM close, and silence at the end still produced an audible click at startup. Click/pop sequencing is therefore a confirmed unresolved driver/DAPM defect; the userspace ordering is a diagnostic mitigation, not a fix.
