...
`alsactl store 0` сохраняет текущую карту в `/var/lib/alsa/asound.state`; проверенные значения записаны для ALSA card ID `Test`. Пользователь наблюдал нулевой/заглушённый микшер в `alsamixer`, тогда как `amixer -c 0 cget` на активной карте показывал HPH 20/24 и switches `on`. Выбор карты и восстановление saved state при холодной загрузке остаются открытыми. `alsa-restore.service` запускался, но сообщил, что несколько read-only impedance controls не разрешают запись.
...
При остановке фильма пользователь слышал треск. Shutdown pop не локализован; нужен отдельный тест последовательности mute/fade, закрытия PCM, выключения PA и последующего reset/power-down.
...
4. **Click-free stop.** Реализовать штатное mute/fade и PA sequencing через DAPM/GPIO consumer: закрыть PCM, проверить отсутствие буферов, выключить PA и только затем освобождать reset/rails. Повторить stop на тоне и фильме без треска.
