...
Service restart and audible playback have been demonstrated during development. On the first `standard3` desktop check, audio was absent because the external modules had not been built and installed for the new kernel release. Installing an old `display1` module was rejected by its different `vermagic`. Rebuilding the complete external set was necessary; using an older LPI provider without clock aliases together with the runtime-PM-aware LPI driver then failed with `LPI clocks unavailable`. The matching provider is now the only version published in the main module directory and supplies the required `core`/`audio` clkdev aliases. After installing the current provider and the permanent card module, restarting the complete audio service restored the card and both SoundWire slaves. A 48 kHz stereo S16_LE controlled tone was heard; the saved HPH 20/24 and RX 78/124 levels were read back, and PipeWire rediscovered the real sink at 50%. The same test confirmed the startup click described above.
...
The generic macro cleanup defects are fixed by patches 68 and 69. RX, TX and VA now balance runtime PM on remove, and VA no longer pins its own module through a permanent `fsgen` consumer reference. On the installed `7.2.4-tcl-acpi-standard3+` kernel all four modules (`snd-soc-lpass-{va,rx,tx}-macro` and `soundwire-qcom`) unloaded completely and loaded again without `Unbalanced pm_runtime_enable`, Oops, trace or refcount warning. The ALSA card and both WCD9385 SoundWire slaves returned, and the user confirmed the controlled 880 Hz stereo playback. The unload still receives two `Unknown cmd 0x100f6` devote replies, so Q6AFE vote/devote remains a separate unresolved protocol issue; the handle and compact-response changes remain in the published series because they are part of the exact hardware-qualified source tree; their presence does not qualify this firmware lifecycle.

The clean `repro1` hardware boot also verified the packaging boundary. The older external LPI provider failed against the runtime-PM-aware LPI driver with `LPI clocks unavailable`; installing the matching provider now published in `patches/kernel/acpi/modules` restored binding without a reboot. The service then registered the ALSA card, both SoundWire slaves became `Attached`, and LPI returned to `runtime_status=suspended`. A stereo 880 Hz test at HPH 12/24 and RX digital 62/124 was audible and left both slaves attached. This pair must be selected together by packaging; silently mixing the two variants is not supported.
...
Основная серия содержит LPI runtime-PM, VA clock balancing и Q6AFE изменения, которые входят в точное проверенное дерево. Для LPI одно лишь `runtime_status=suspended` раньше не подтверждало управление clocks: ACPI-путь пропускал заполнение OF PM clock list. После адаптации LPI driver и согласованный внешний provider с aliases проверены на ноутбуке: `on` активирует RX MCLK/NPL, `auto` возвращает `suspended` с нулевыми enable counts, PCM-тест проходит. Ответы firmware `Unknown cmd` на Q6AFE vote/devote остаются нерешёнными. Полноценный system suspend/resume и замена power-hold на regulator consumers/DAPM остаются отдельными требованиями.
