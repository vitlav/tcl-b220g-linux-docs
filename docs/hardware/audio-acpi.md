...
The Q6AFE remote LPASS hardware-vote lifecycle is not yet correct on this firmware. At early boot, VA block 3 requested opcode `0x100f4`; firmware returned `APR_BASIC_RSP_RESULT` status `0x16` (`Unknown cmd`) and that probe ended with `-ETIMEDOUT`. This was initially misidentified as a compact vote response. After APR became fully ready, RX/TX/VA macro were bound and playback worked, so the early result alone does not prove that every vote is unsupported. A controlled late hot-unload then produced two `Unknown cmd 0x100f6` devote replies, VA remained in use, and reprobed RX/TX reported `Unbalanced pm_runtime_enable`. The card and both SoundWire slaves were restored by the normal service restart, followed by successful PipeWire playback at 50%. Q6AFE handle/compact-response candidates remain excluded from the working TCL patch series until vote/devote and driver cleanup pass together.
