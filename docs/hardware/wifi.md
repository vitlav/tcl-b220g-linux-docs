...
Встроенный WLAN использует Qualcomm WCN3990 через SNOC и драйвер
`ath10k_snoc`. Поддержка проверена при DT-загрузке Linux 6.18.34 и при
ACPI-загрузке Linux 7.2.4 с описанными в этом репозитории адаптациями.
...
| ACPI и заводской MAC | После перезагрузки MPSS `running`, координатор `stage=4`/`ready=Y`, `wlan0` и DHCP работают с `c8:2a:f1:7f:78:60` | Требуется ранняя SSDT; адрес относится к проверенному экземпляру |
...
OEM ACPI не публикует MAC, поэтому без дополнительного firmware-свойства
ath10k выбирает новый локально администрируемый адрес при каждой загрузке.
Заводской MAC хранится в GPT-разделе `DPP`, в RWFS-файле
`WLAN.PROVISION`; на проверенном экземпляре это `c8:2a:f1:7f:78:60`, что
совпадает с адресом физического адаптера в Windows. [Генератор ранней
SSDT](../../system/acpi/early-ssdt/README.md) передаёт индивидуальный адрес через
стандартное свойство `mac-address`. В проверенной ACPI-загрузке адрес `wlan0`
точно совпал с DPP,
предупреждение `invalid MAC address; choosing random` исчезло, а DHCP выдал
адрес штатно. Значение должно быть ACPI Package из шести целых: Buffer делает
недействительным весь стандартный `_DSD`, включая `qcom,msa-fixed-perm`, и
останавливает WLAN coordinator на stage 3. SSID и ключи задаются для конкретной
установки. Скрипт
[prefer-5ghz.py](../../research/peripherals/ubuntu-base/prefer-5ghz.py)
иллюстрирует два профиля, но содержит фиксированный SSID и требует адаптации;
он не доказывает поддержку 802.11r.

Если cfg80211/ath10k загружаются из initramfs, туда должны входить
`regulatory.db` и `regulatory.db.p7s`. Наличие файлов только в корневой системе
слишком поздно: журнал уже содержит `failed to load regulatory.db`, а повторной
автоматической загрузки после switch-root нет. В ALT это задаётся штатным
`install_items` в конфигурации dracut. После исправления надо проверить
regdomain и повторяющееся firmware-сообщение `chan info: invalid frequency 0
(idx 41 out of bounds)`; текущая успешная ассоциация на 5 GHz не объясняет эту
ошибку.
