...
**Проверено на оборудовании:** инициализация UFS, чтение GPT всех шести LUN, полное чтение каждого LUN при архивировании и проверка извлечённого содержимого по SHA256 выполнены в DT-запуске. ICE прошёл отдельную проверку инициализации и возможностей. ACPI-запуск также проверен: `QCOM24A5:00` получил IORT input ID `0x81030000`, был добавлен в Apps SMMU group 0, после чего обнаружены все шесть LUN с теми же размерами и моделью Samsung. Зашифрованное I/O, запись и восстановление накопителя не испытаны.
...
### GPT основного LUN (`/dev/sdb`)

Проверено чтением устройства через ACPI-запуск, без записи. Размер логического сектора — 4096 байт, usable LBA `6–61360122`, резервная GPT — LBA `61360127`. GUID диска: `a8623bd4-d854-4278-bd21-c66f5a75edc5`.

| Раздел | LBA | Размер | Тип | Имя |
|---:|---:|---:|---|---|
| 1 | 256–66815 | 260 MiB | EFI System | EFI system partition |
| 2 | 66816–70911 | 16 MiB | Microsoft Reserved | Microsoft reserved partition |
| 3 | 70912–35503871 | 135.2 GiB | Microsoft Basic Data | Basic data partition |
| 4 | 61103872–61359871 | 1 GiB | Windows Recovery | Basic data partition |

Промежуток между разделами 3 и 4 не изменён; перед установкой JeOS требуется отдельное решение о размещении Linux-разделов.
...
ACPI уже перечисляет UFS как `QCOM24A5:00` (`\_SB_.UFS0`), поэтому сначала нужно включить в конфигурации `SCSI_UFSHCD`, `SCSI_UFSHCD_PLATFORM` и `SCSI_UFS_QCOM`, затем проверить привязку `ufshcd-qcom`. Текущий драйвер содержит ACPI match `QCOM24A5`, но часть Qualcomm UFS и ICE всё ещё использует OF-запросы (PHY, interconnect, ICE и частотную таблицу); их совместимость с ACPI должна быть проверена по probe, а не предположена. До этой проверки внутренний диск не размечать. После появления UFS повторно снять GPT и только затем готовить JeOS/GRUB.
