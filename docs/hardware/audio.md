...
**Проверено на оборудовании:** WCD9385 идентифицирован прямым чтением регистров. Через DT подтверждены playback и PulseAudio/Firefox на ядре `6.18.34-tcl-audio2`; отдельная ACPI-проверка на `7.2.4-tcl-acpi-display1+` также дала слышимый звук тона и фильма. Для ACPI-пути установлен внешний ASoC-модуль, сохранён mixer-профиль, а systemd автоматически запускает ADSP и регистрирует playback-карту. Полный автоматический холодный старт пока зависит от неподтверждённой схемы питания/reset WCD9385; подробности и точный статус приведены в [ACPI-аудиоспецификации](audio-acpi.md).
...
Проверка слышимого звука подтверждает работу этого тракта в целом. DT-ресурсы и исторический DT-путь остаются в этом разделе; состояние ACPI boot, проверенный mixer profile и план интеграции вынесены в [отдельную ACPI-спецификацию](audio-acpi.md). Независимое сопоставление каждого канала отдельному динамику, характеристики усилителя и измерение аналогового сигнала не выполнены.
...
Для слышимого выхода нужны HPHL/HPHR/CLSH switches, RX/WCD DAPM-маршруты и включение PA. Состояние PCM RUNNING само по себе не доказывает аналоговый звук. ACPI playback и точные подтверждённые controls описаны в [ACPI-аудиоспецификации](audio-acpi.md). На ACPI-пути пользователь подтвердил слышимый тон и фильм; fade/PA/PCM порядок проверен, но отсутствие щелчка при остановке ещё не подтверждено на слух.
...
Временный ACPI-тест зарегистрировал отдельный Q6ASM `MultiMedia2` capture frontend; восьмисекундная запись завершилась, однако ожидаемый тестовый тон в WAV не обнаружен. Физическая разводка аналоговых микрофонов/DMIC, расположение и назначение входов не подтверждены. Подробности — в [ACPI-аудиоспецификации](audio-acpi.md).

DT playback проверен. Для ACPI playback-карта, DAPM PA, сохранение mixer-профиля и запуск ADSP/карты через systemd настроены; остаются штатное питание/reset и автоматическое SoundWire-подключение кодека, затем холодный старт, suspend/resume и акустическая проверка stop без щелчка. Подтверждённый слышимый звук сам по себе не закрывает эти требования.
