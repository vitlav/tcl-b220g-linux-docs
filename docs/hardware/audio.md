...
**Проверено на оборудовании:** WCD9385 идентифицирован прямым чтением регистров. Через DT подтверждены playback и PulseAudio/Firefox на ядре `6.18.34-tcl-audio2`; отдельная ACPI-проверка на `7.2.4-tcl-acpi-display1+` также дала слышимый звук тона и фильма. Оба пути пока используют диагностическую ASoC-карту и ручную подготовку; они описаны отдельно в [ACPI-аудиоспецификации](audio-acpi.md). Постоянная интеграция питания, маршрутов и автозапуска ещё не завершена.
...
Для постоянной поддержки нужны полные DT- и ACPI ASoC-карты, штатное владение GPIO и regulators, PA sequencing через DAPM, проверка каналов и уровней, runtime PM, suspend/resume, click-free stop и повторное воспроизведение. Подтверждённый слышимый звук не закрывает эти требования.
