...
Используется собственная сборка Linux `6.18.34-tcl-audio2` с изменениями DT, экспериментальными драйверами и отдельными исправлениями аудио/графики. Пользовательское окружение — Ubuntu 26.04.1 LTS ARM64, на основе Ubuntu Base.

Ядро собиралось на x86_64 с `ARCH=arm64 CROSS_COMPILE=aarch64-linux-gnu-`; GCC 15.3.1 предоставлен RPM ALT Sisyphus `gcc-aarch64-linux-gnu-15.3.1-alt4.x86_64`. Принадлежность компилятора пакету проверена; способ первоначальной установки по RPM-базе не определяется. Полная кросс-сборка Image и модулей выполнена.
...
Для ACPI полностью собраны Image и 948 модулей `6.18.34-tcl-acpi2` с SCM ACPI v2 и раздельным выбором Apps/Adreno SMMU. [Состав, конфигурация и проверки](../research/acpi-boot/acpi2/README.md). Комплект не установлен и не проверен аппаратно.

## Изменения образа Ubuntu

[Спецификация Ubuntu](ubuntu.md) описывает состав системы, firmware и порядок запуска Qualcomm, настройки DHCP/SSH и размещение rootfs. Образцы конфигурации имеют ограниченную применимость, указанную рядом с исходниками.
...
## Требуемые изменения ядра

[Патчи ядра](../patches/kernel/README.md) собраны отдельно с назначением, статусом и SHA256. Полная последовательная серия для чистой базы ещё не сформирована.
