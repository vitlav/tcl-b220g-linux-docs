...
Используется собственная сборка Linux `6.18.34-tcl-audio2` с изменениями DT, экспериментальными драйверами и отдельными исправлениями аудио/графики. Пользовательское окружение — Ubuntu 26.04.1 LTS ARM64, на основе Ubuntu Base. Нельзя называть такой комплект «чистым ядром Ubuntu».
...
Для ACPI полностью собраны Image и 948 модулей `6.18.34-tcl-acpi2` с SCM ACPI v2 и раздельным выбором Apps/Adreno SMMU. [Состав, конфигурация и проверки](../research/acpi-boot/acpi2/README.md). Комплект не установлен и не проверен аппаратно. Не путать его с последней ACPI1 Image на USB.
