...
Рабочая Ubuntu использует Device Tree. На Linux 7.2.4 с [проверенной ACPI-серией](../patches/kernel/acpi/README.md) подтверждены диагностическая загрузка без DT, SCM/SMMU, USB с проверенной записью логов, Wi-Fi 5 ГГц и SSH; HID-клавиатура зарегистрирована. UFS через ACPI также проверен: после сопоставления IORT-компонента `\\_SB.UFS0` с `QCOM24A5` накопитель добавляется в Apps SMMU и перечисляет все шесть LUN. Полноценная Ubuntu и нативная графика через ACPI ещё не подтверждены.
...
[Проверка функций IORT](../research/acpi-audit/iort-mappings/README.md) подтвердила первые mappings всех 16 узлов с явным ID. На оборудовании отдельно подтверждён UFS DMA: после сопоставления имени IORT `\\_SB.UFS0` с ACPI HID `QCOM24A5` устройство получило Apps SMMU SID `0xa0` и перечислило LUN. Это host-проверка, не тест записи или ICE-шифрования. Flags=0 сами по себе не означают нарушение спецификации. Глобальная установка SINGLE_MAPPING не обоснована: есть несколько mappings и диапазоны.
