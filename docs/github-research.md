...
## Подтверждённые результаты для внешнего отчёта

Актуальные сведения — [сводка поддержки](status.md) и [спецификации подсистем](subsystems.md). Исторический статус внешних обсуждений выше не описывает текущие результаты этого проекта.

- Ubuntu 26.04.1 LTS ARM64 и модифицированное ядро Linux 6.18.34-tcl-audio2; точная применимость определяется конфигурацией и патчами.
- Wi-Fi 5 ГГц и автоматический SSH, чтение шести UFS LUN, CPUFreq и температурная телеметрия.
- Нативный MSM/DPU-вывод через LT8911EXB handoff, Adreno 618 OpenGL/Vulkan и Wayland; полный cold-start моста ещё не реализован.
- Звук WCD9385 и изображение камеры подтверждены; постоянная аудиоинтеграция и микрофон остаются открытыми.
- Venus H.264 проверен с известными ошибками аппаратного пути; полной матрицы codecs/profiles нет.
- ACPI остаётся отдельным незавершённым портом; SCM/SMMU подготовлены и собраны, аппаратно не проверены.

Внешний отчёт должен ссылаться на конкретные [патчи](../patches/kernel/README.md) и спецификации, указывать границы проверки. Публикация документации не означает принятие изменений upstream. Персональные UFS-образы, ключи и credentials не являются материалами такого отчёта.
