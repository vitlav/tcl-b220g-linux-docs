...
Значение `mac-address` записывается как ACPI `Package` из шести целых чисел,
а не как `Buffer`. Проверенный Linux 7.2 отклоняет весь стандартный `_DSD`
property package, если одно из его значений имеет тип Buffer. В результате
теряется также обязательное `qcom,msa-fixed-perm`, координатор WLAN завершает
stage 3 с `-EINVAL`, а MPSS и `wlan0` не запускаются. Генератор проверяет тип
контейнера до компиляции AML.
